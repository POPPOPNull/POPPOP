...
import { selectPopularPopupByMonth } from '../../api/adminAPI';
    ...
    useEffect(() => {
        const fetchChartData = async () => {
            try {
                ...
                const popularPopups = await selectPopularPopupByMonth(selectedMonth);

                console.log("API 테스트", popularPopups);

                if (popularPopups && popularPopups.length > 0) {

                    const labels = popularPopups.map(item => item.popupName);
                    const data = popularPopups.map(item => item.eventCount);
...
export default AdminManagerBarChart;